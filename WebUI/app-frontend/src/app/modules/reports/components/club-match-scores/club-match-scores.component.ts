...
import { ClubMonthlyMatchScoreStatistics } from 'src/app/models/report/club-monthly-match-score-statistics.model';
import { MatchScoreStatistics } from 'src/app/models/report/match-score-statistics.model';
import { TranslationProvider } from 'src/app/providers/translation-provider.model';
import { ReportService } from 'src/app/services/report.service';

@Component({
  selector: 'app-club-match-scores',
  templateUrl: './club-match-scores.component.html',
  styleUrls: ['./club-match-scores.component.scss']
})
export class ClubMatchScoresComponent implements OnInit {

  form: FormGroup;
  months :string[] = [];
  
  scores : Array<MatchScoreStatistics>;
  scoresMonthly: ClubMonthlyMatchScoreStatistics;
  ...
  ngOnInit(): void {
    this.createForm();
    this.spinner.show();
    this.reportService.getClubMatchScores().then(res => {
      this.scores = res;
      this.spinner.hide();
    })
    .catch(error => {
  ...
  onScoreTypeSelect($event){
    if(this.selectedMatchScoreType != null){
      let score = this.scores.find(x => x.scoreType == this.selectedMatchScoreType);
  ...
  getStatistics(startDate: string, endDate: string){
    this.spinner.show();
    this.reportService.getClubMatchScoresMonthly(startDate, endDate)
    .then(res => {
