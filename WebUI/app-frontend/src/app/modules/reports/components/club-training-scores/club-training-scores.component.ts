...
import { ClubMonthlyTrainingScoreStatistics } from 'src/app/models/report/club-monthly-training-score-statistics.model';
import { TrainingScoreStatistics } from 'src/app/models/report/training-score-statistics.model';
import { TranslationProvider } from 'src/app/providers/translation-provider.model';
import { ReportService } from 'src/app/services/report.service';

@Component({
  selector: 'app-club-training-scores',
  templateUrl: './club-training-scores.component.html',
  styleUrls: ['./club-training-scores.component.scss']
})
export class ClubTrainingScoresComponent implements OnInit {

  form: FormGroup;
  months :string[] = [];
  
  scores : Array<TrainingScoreStatistics>;
  scoresMonthly: ClubMonthlyTrainingScoreStatistics;
  ...
  ngOnInit(): void {
    this.createForm();
    this.spinner.show();
    this.reportService.getClubTrainingScores().then(res => {
      this.scores = res;
      this.spinner.hide();
    });
  }
  ...
  checkView($event: any){
    if(this.monthlyView && this.scoresMonthly == null && this.selectedTrainingScoreType){
      let now = new Date();
  ...
  onScoreTypeSelect($event){
    if(this.selectedTrainingScoreType != null){
      let score = this.scores.find(x => x.scoreType == this.selectedTrainingScoreType);
  ...
  getStatistics(startDate: string, endDate: string){
    this.spinner.show();
    this.reportService.getClubTrainingScoresMonthly(startDate, endDate)
    .then(res => {
